     ...
     List<Metric> extractMetricsFromApiResponse (Stat stat, JsonNode jsonNode) {
         ...
         if(stat.getStats()!=null){
             for(Stat childStat: stat.getStats()) {
                 extractMetricsFromApiResponse(childStat, JsonUtils.getNestedObject(jsonNode, childStat.getType()));//todo:check this
             }
         }
         for (com.appdynamics.extensions.couchdb.config.Metric metricFromConfig : stat.getMetric()) {


             String value = JsonUtils.getNestedObject(jsonNode, metricFromConfig.getAttr()).get("value").toString();
             if (value != null) {
